"""VaR calculation and persistence orchestration services.
...
class HistoricalVaRAndESCalculationResult:
    """Result of combined historical VaR and ES calculation and persistence workflow.

    Attributes
    ----------
    var_result : HistoricalVaRResult
        In-memory historical VaR calculation result. Also persisted to VaRResult table.
    es_result : HistoricalExpectedShortfallResult
        In-memory historical ES calculation result. Also persisted to ExpectedShortfallResult table.
    """
...
class HistoricalVaRCalculationService:
    ...
    def __init__(self, session_factory: SessionFactory) -> None:
        ...
        self.es_engine = HistoricalExpectedShortfall()
    ...
    def calculate_and_persist_historical_var_and_es(
        self,
        portfolio: RiskReadyPortfolio,
        historical_price_points: list[PricePoint],
        risk_methodology: RiskMethodology,
        ...
        """Calculate and persist both historical VaR and ES under the same CalculationRun.

        Manages the complete workflow from price data to persisted VaR and ES results.
        Creates a single CalculationRun for lineage and audit trail.
        ...
        -------
        HistoricalVaRAndESCalculationResult
            In-memory results (both VaR and ES). Also persisted to respective tables.
        ...
        -----
        On calculation failure after CalculationRun creation: marks run as FAILED,
        does not insert partial ES result (VaR may be partially inserted; transaction
        rollback depends on session scope), and re-raises the exception.
        """
        portfolio_fund_id = int(portfolio.fund_id)
        portfolio_valuation_date = date.fromisoformat(portfolio.valuation_date)
        ...
        try:
            ...
            # Calculate historical ES using the matched VaR result
            es_input = HistoricalExpectedShortfallInput(
                portfolio=portfolio,
                scenario_pnls=scenario_result.scenario_pnls,
                var_result=var_result,
            )
            es_result = self.es_engine.calculate(es_input)

            # Map and persist ExpectedShortfallResult
            orm_es_result = map_historical_es_result_to_orm(
                es_result,
                calculation_run_id=calculation_run_id,
            )
            es_repo = ExpectedShortfallResultRepository(self.session_factory)
            es_repo.insert(orm_es_result)

            # Mark calculation run COMPLETED
            calc_repo.update_status(calculation_run_id, CalculationStatusEnum.COMPLETED)

            return HistoricalVaRAndESCalculationResult(
                var_result=var_result,
                es_result=es_result,
            )
...
class ParametricNormalVaRCalculationService:
    """Orchestrate parametric normal VaR calculation and persistence.

    Workflow:
    1. Create CalculationRun with status RUNNING
    2. Convert prices to returns
    3. Generate equity scenario P&Ls
    4. Calculate parametric normal VaR
    5. Map and persist VaRResult
    6. Mark CalculationRun COMPLETED
    7. On failure: mark FAILED and re-raise
    ...
    def __init__(self, session_factory: SessionFactory) -> None:
        ...
        self.price_converter = PriceToReturnConverter()
        self.scenario_generator = EquityScenarioPnLGenerator()
        self.var_engine = ParametricNormalVaR()

    def calculate_and_persist_parametric_normal_var(
        self,
        portfolio: RiskReadyPortfolio,
        historical_price_points: list[PricePoint],
        risk_methodology: RiskMethodology,
        ...
        """Calculate parametric normal VaR and persist result.
        ...
        portfolio : RiskReadyPortfolio
            Enriched portfolio at valuation date.
        historical_price_points : list[PricePoint]
            Historical prices for scenario generation (typically 250+ observations).
        risk_methodology : RiskMethodology
            Risk parameters (confidence level, lookback days, horizon, etc.).
        position_snapshot_id : int
            FK to PositionSnapshot for lineage.
        ...
        try:
            ...
            var_input = ParametricNormalVaRInput(
                portfolio=portfolio,
                confidence_level=risk_methodology.var_confidence_level,
                horizon_days=risk_methodology.var_horizon_days,
                scenario_pnls=scenario_result.scenario_pnls,
            ...
            # Map to ORM and persist
            orm_var_result = map_parametric_normal_var_result_to_orm(
                var_result,
                calculation_run_id=calculation_run_id,
                lookback_days=risk_methodology.var_lookback_days,
            )
